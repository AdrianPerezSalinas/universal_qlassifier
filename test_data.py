...
'''
def Accuracy_test_worldmap(theta, alpha, test_data, reprs, entanglement, chi, weights=None):
    """
    This function takes the parameters of a solved problem and one data computes how many points are correct
    INPUT: 
        -theta: initial point for the theta parameters. The shape must be correct (qubits, layers, 3)
    ...
    solutions = np.empty((len(test_data), 2 + 3)) #data  #Esto se podrá mejorar en el futuro
    for i, d in enumerate(test_data):
        x, y = d
        y_, angles = _claim_worldmap(theta, alpha, weights, x, reprs, entanglement, chi)
        solutions[i,:2] = angles
        solutions[i, -3] = y
        solutions[i, -2] = y_
        solutions[i, -1] = int(y == y_)
...
def _claim_worldmap(theta, alpha, weight, x, reprs, entanglement, chi):
    """
    This function takes the parameters of a solved problem and one data computes classification of this point
    INPUT: 
        -theta: initial point for the theta parameters. The shape must be correct (qubits, layers, 3)
    ...
    if chi == 'fidelity_chi':
        y_, angles = _claim_fidelity_worldmap(theta, alpha, x, reprs, entanglement)
        
    if chi == 'weighted_fidelity_chi':
        y_, angles = _claim_weighted_fidelity_worldmap(theta, alpha, weight, x, reprs, entanglement)
        
    return y_, angles   
        
    
def _claim_fidelity_worldmap(theta, alpha, x, reprs, entanglement):
    """
    This function is inside _claim for fidelity_chi
    INPUT: 
        -theta: initial point for the theta parameters. The shape must be correct (qubits, layers, 3)
    ...
    Fidelities = [fidelity(r, C.psi) for r in reprs]
    b_a = C.psi[1] / C.psi[0]
    angles = np.array([np.angle(b_a), 2 * np.arctan(np.abs(b_a))])
    
    return np.argmax(Fidelities), angles


def _claim_weighted_fidelity_worldmap(theta, alpha, weight, x, reprs, entanglement):
    """
    This function is inside _claim for weighted_fidelity_chi
    INPUT: 
        -theta: initial point for the theta parameters. The shape must be correct (qubits, layers, 3)
    ...
    """
    theta_aux = code_coords(theta, alpha, x)
    fids, C = mat_fidelities(theta_aux, weight, reprs, entanglement, return_circuit=True)
    b_a = C.psi[1]/C.psi[0]
    angles = np.array([np.angle(b_a), 2 * np.arctan(np.abs(b_a))])
    w_fid = w_fidelities(fids, weight)    
    #angles = np.array([2 * np.arctan(np.abs(C.psi[1] / C.psi[0])), np.angle(C.psi[1] / C.psi[0])])
        
    return np.argmax(w_fid), angles
